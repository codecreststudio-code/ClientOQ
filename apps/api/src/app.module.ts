      ...
      secret: (() => {
        ...
        if (!secret || secret.includes('development') || secret.length < 32) {
          if (process.env.NODE_ENV === 'production') {
            throw new Error('JWT_SECRET must be a strong 32+ char secret in production. Set it in your environment.');
          }
          console.warn('[WARN] Using weak JWT_SECRET — set a strong one before deploying to production!');
        }
        return secret || 'super-secret-key-clientoq-ai-2026-development';
      })(),
